...
class PurePursuit:
    def __init__(self):
        ...
        rospy.Subscriber(path_name, Path, self.path_callback)
        ...
        while not rospy.is_shutdown():
            if self.is_path and self.is_odom and self.is_status:
                ...
                if self.is_look_forward_point:
                    ...
                    rospy.loginfo("No valid forward point found")
    ...
    def calc_pure_pursuit(self):
        ...
        dis = None  # Initialize dis to ensure it is defined
        ...
        if dis is not None and self.is_look_forward_point:
            # Steering 각도 계산
            theta = atan2(local_path_point[1], local_path_point[0])
            steering = atan2(2 * self.vehicle_length * sin(theta), dis)
        else:
            rospy.logwarn("No valid forward point found or dis is not set.")
            steering = 0.0  # Default steering value if no valid point is found
