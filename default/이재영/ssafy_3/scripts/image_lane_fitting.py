...
class IMGParser:
    def __init__(self, pkg_name = 'ssafy_3'):
        ...
        '''
        curve_learner = CURVEFit(order=3, lane_width=3.5, y_margin=0.5, x_range=10, min_pts=5, alpha=0.1, dx=1.0)
        #END
        rate = rospy.Rate(10)
...
class CURVEFit:    
    def __init__(self, order, alpha, lane_width, y_margin, x_range, dx, min_pts):
        ...
        self.ransac_left = linear_model.RANSACRegressor(
            estimator=linear_model.Lasso(alpha=0.1),  # Adjust alpha as needed
            max_trials=100,  # Adjust based on your data
            min_samples=self.min_pts,
            residual_threshold=self.y_margin,
            loss='absolute_error'
        )
